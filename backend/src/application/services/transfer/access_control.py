...
class TransferProfileAccessControlService:
    ...
    @staticmethod
    def can_get_transfers(account_owner_id: int, requesting_user: UserAccessDTO) -> Optional[bool]:
        if account_owner_id == requesting_user.id and UserRole(requesting_user.role) in [UserRole.CLIENT]:
            return True
        raise ForbiddenError()

    @staticmethod
    def can_create_transfer(account_owner_id: int, requesting_user: UserAccessDTO) -> Optional[bool]:
        if account_owner_id == requesting_user.id and UserRole(requesting_user.role) in [UserRole.CLIENT]:
            return True
        raise ForbiddenError()
